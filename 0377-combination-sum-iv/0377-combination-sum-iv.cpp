...
//             return 1;
//         if(dp[target] != -1) 
//             return dp[target]; //IF WE ALREADY CALCULATED THE TARGET
...
// class Solution {
// public:
//     int noOfWays(vector<int> &nums, int target, int size){
//         if(target < 0) 
//             return 0;
//         if(target == 0) 
//             return 1;
//         int ways = 0; 
//         for(int i = 0; i < size; i++)
//             ways += noOfWays(nums, target - nums[i], size);
//         return ways;
//     }
//     int combinationSum4(vector<int>& nums, int target) {
//         return noOfWays(nums, target, nums.size());
//     }
// };